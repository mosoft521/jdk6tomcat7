/**
 * @Title: SensitizeUtil.java
 * @Package com.kakaloans.micro.credit.common.util
 * @Description:
 * @author zhangjiawen
 * @date 2017年8月17日 下午1:39:11
 * @version V1.0
 */
package com.kakaloans.micro.credit.common.util;

import com.kakaloans.micro.credit.common.annotation.Sensitize;
...
public class SensitizeUtil {
     ...
     * 日志工具
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(SensitizeUtil.class);
    private static Map<String, String> channelRequestEnumMap = ChannelRequestEnum.getWholeMap();
     ...
     * @Fields FAIL : 访问异常
     */
    public static String sensitize(String enumName, String mapKeyParam) {
        String mapKey;
        if (null != mapKeyParam) {
        ...
        //反射取得Enum
        String enumNameKey = channelRequestEnumMap.get(enumName);
        String packageName = "com.kakaloans.micro.credit.common.enums.request.";
        ...
        try {
            ...
            String s = (String) staticMethod.invoke(clazz, mapKey);
            return s;
     ...
     * @Fields FAIL : 访问异常
     */
    public static String sensitize(ChannelRequestEnum en, String mapKeyParam) {
        String mapKey;
        if (null != mapKeyParam) {
            mapKey = StringUtils.trim(mapKeyParam);
        } else {
        ...
        //特殊处理
        if (en == ChannelRequestEnum.BigPercentageGrade) {
            //百万
            // 按百分制记
            //先按大范围分为10档
            ...
            if (0 == n) {
                ...
                n -= 2;
                int nx = n / 10;//0,1,2,...,10
                int xSn = 10000 * nx * nx;
                int dSn = 10000 + nx * 20000;
                int ny = n % 10; //0,1,2,...,9
                int ySn = dSn / 10 * ny;
                return Integer.toString(xSn + ySn);
            }

        } else if (en == ChannelRequestEnum.SmallPercentageGrade) {
            //十万
            //按百分制记
            //先按大范围分为10档
            ...
            if (0 == n) {
                ...
                //采用数学公式优化之
                int nx = n / 10;//0,1,2,...,10
                int xSn = 1000 * nx + nx * (nx - 1) * 1000;
                int dSn = 1000 + nx * 2000;
                int ny = n % 10; //0,1,2,...,9
                int ySn = dSn / 10 * ny;
                return Integer.toString(xSn + ySn);
            }
    ...
    public static void sensitize(Object obj) {
        Class<?> clazz = obj.getClass();
        Class<? extends Annotation> annClazz = Sensitize.class;
        String type = "Sensitize";
        ...
        if ((annWFMap != null) && ((annWFMap.size() > 0))) {
            ...
            while (itPF.hasNext()) {
                ...
                if (type.equals("Sensitize")) {
                    LOGGER.debug(ety.getKey().toString());
                    LOGGER.debug(ety.getKey().getName() + " = " + ((Sensitize) anno).channel() + " - " + ((Sensitize) anno).value());
                    try {
                        //获取私有的字段的值
                        String mapKey = "";
                        ...
                        String ret = SensitizeUtil.sensitize(((Sensitize) anno).value(), mapKey);
                        ...
                        if (ety.getKey().getType().equals(String.class)) {
                            ...
                            ety.getKey().set(obj, Integer.valueOf(ret));
                        } else if (ety.getKey().getType().equals(Long.class)) {
...
//        creditCardInfo.setCurrency("1");
//        SensitizeUtil.sensitize(creditCardInfo);
